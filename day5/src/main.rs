...
pub fn rearrange(mut stacks: Vec<Vec<char>>, instruction: &str) -> Vec<Vec<char>> {
    ...
    let src: usize = inst[3].parse::<usize>().unwrap();
    let dst: usize = inst[5].parse::<usize>().unwrap();

    println!("items to move {}, from {} to {}", item_cnt, src, dst);

    for n in 0..item_cnt {
        let target = stacks[src - 1].pop().unwrap();
        stacks[dst - 1].push(target);
    }
    stacks
}
...
fn main() {
    ...
    // dbg!(&subs);
    let mut stack_container: Vec<Vec<char>> = vec![];
    for _n in 0..stackcount {
        stack_container.push(vec![]);
    }
